...
-- Chinese (Trad.)
CREATE VIEW Language_zh_TW AS SELECT Tag, Text, Gender, Plurality from LocalizedText where Language = 'zh_TW';
CREATE TRIGGER Language_zh_TW_insert INSTEAD OF INSERT ON Language_zh_TW BEGIN insert into LocalizedText(Language, Tag, Text, Gender, Plurality) VALUES('zh_TW', NEW.Tag, NEW.Text, NEW.Gender, NEW.Plurality); END;
CREATE TRIGGER Language_zh_TW_update INSTEAD OF UPDATE ON Language_zh_TW BEGIN UPDATE LocalizedText SET Text = NEW.Text , Gender = NEW.Gender, Plurality = NEW.Plurality WHERE Language = 'zh_TW' and Tag = NEW.Tag; END;
CREATE TRIGGER Language_zh_TW_delete INSTEAD OF DELETE ON Language_zh_TW BEGIN DELETE from LocalizedText where Language = 'zh_TW' and Tag = OLD.Tag; END;								
